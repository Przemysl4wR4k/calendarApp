...
export class AppointmentService {
  private appointments: Appointment[] = [
    ...
    { id: 4, title: '2 day task', start: new Date(new Date().getTime() + 8 * 60 * 60 * 1000), end: new Date(new Date().getTime() + 56 * 60 * 60 * 1000 + 1 * 60 * 1000) }
  ];
  ...
  private filterAppointments(appointments: Appointment[], selectedDate: Date): Appointment[] {
    ...
    return appointments.map(appointment => {
      ...
      const adjustedStart = startDate < selectedDateStart ? selectedDateStart : startDate;
      const adjustedEnd = endDate > selectedDateEnd ? selectedDateEnd : endDate;
